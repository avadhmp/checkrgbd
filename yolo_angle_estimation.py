...
def run():
    ...
    c = profile.get_stream(rs.stream.color).as_video_stream_profile().get_intrinsics()
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    text1 = "x : " + str(round(Xtarget))
                    text2 = "y : " + str(round(Ytarget))
                    text3 = "z : " + str(round(Ztarget))
                    ...
                    cv2.putText(img0, text3, (int((xyxy[0] + xyxy[2])/2) - 40, int((xyxy[1] + xyxy[3])/2) + 80), cv2.FONT_HERSHEY_PLAIN, 3, [225, 255, 255], thickness=2, lineType=cv2.LINE_AA)
